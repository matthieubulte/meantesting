...
class BuresWassersteinN(BuresWasserstein):
    @classmethod
    def exp(cls, A, B):
        return (jnp.eye(A.shape[0]) + B) @ A @ (jnp.eye(A.shape[0]) + B)
    ...
    @classmethod
    def log(cls, A, B):
        return _T(A, B) - jnp.eye(B.shape[0])

    @classmethod
    def dist(cls, A, B):
        sqrt_A = _sqrt(A)
        return jnp.sqrt(jnp.trace(A + B - 2 * _sqrt(sqrt_A @ B @ sqrt_A)))
    ...
    def frechet_mean(
        ...
        def body_fn(carry, t):
            ...
            key, subkey = jax.random.split(key)
            K_t = X[jax.random.randint(subkey, (), 0, X.shape[0])]
            G_hat_t = (1 - eta_t) * Id + eta_t * _T(Sigma_t, K_t)
            Sigma_t = sym(G_hat_t @ Sigma_t @ G_hat_t)
            return (Sigma_t, key), None
...
@jax.jit
def _sqrt(A):
    eigenvals, eigenvecs = jax.numpy.linalg.eigh(A)
    eigenvals = jnp.maximum(eigenvals, 0)
    sqrt_eigenvals = jnp.sqrt(eigenvals)
    sqrt_D = jnp.diag(sqrt_eigenvals)
    return eigenvecs @ sqrt_D @ eigenvecs.T


def _T(A, B):
    sqrt_A = _sqrt(A)
    sqrt_A_inv = jnp.linalg.inv(sqrt_A)
    return sqrt_A_inv @ _sqrt(sym(sqrt_A @ B @ sqrt_A)) @ sqrt_A_inv
